...
import com.github.llcawthorne.datamodel.{DataModel, Priority}
import com.github.llcawthorne.queries.Queries._
...
class QueriesSpec extends FunSpec with Matchers with BeforeAndAfterAll {
  ...
  override protected def afterAll(): Unit = Await.result(db.shutdown, 2 seconds)
  ...
  describe("Task Data Model Query Spec") {
    ...
    it("should select all high priority task titles") {
      val highPriorityTasks = performAction(selectHighPriorityTaskTitleQuery.result)
    ...
    it("should find all due tasks") {
      val dueTasks = performAction(selectAllTasksDue.result)
    ...
    it("should find all tasks due today") {
      val dueTasks = performAction(selectAllTaskTitleDueToday.result)
