package com.github.llcawthorne.sentiment
...
object SentimentAnalyzer {
  ...
  /**
    ...
    * @return List of sentences paired with sentiment values in numeric form
    */
  def extractSentimentsNumeric(text: String): List[(String, Int)] = {
    val annotation: Annotation = pipeline.process(text)
    val sentences = annotation.get(classOf[CoreAnnotations.SentencesAnnotation]).asScala
    sentences
      .map(sentence => (sentence, sentence.get(classOf[SentimentCoreAnnotations.SentimentAnnotatedTree])))
